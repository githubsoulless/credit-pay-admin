...
<body onload="init()">
	<form action="${ctx }/info/update" id="subForm" method="post" enctype="multipart/form-data"
	 target="hidden_frame">
		...
		<div class="begin">
			<table border="" cellspacing="" cellpadding="">
				<tr>
					...
					<td><span><input name="title" id="title" value="${information.title}" maxlength="10" class="ipt"/>
					<input name="infoId" id="infoId" value="${information.infoId }" type="hidden" class="ipt" />
					</span></td>
				</tr>
				<tr>
					<td class="width90"><span style="color:red;">*</span><span>缩略图：</span></td>
					<td><span><img src="${ctx}/download/infoImg/${information.infoId }"  height="20px" width="10px">
				...
				<tr>
					...
					<td><span>
						<select name="status" id="status" >
							<option value="0" <c:if test="${information.status == 0 }">selected="selected"</c:if>>使用中</option>
							<option value="1" <c:if test="${information.status == 1 }">selected="selected"</c:if>>禁用</option>
						</select>
					</span></td>
				</tr>
				<tr style="height: 80px;width: 730px">
					...
					<td><span>
					<script id="content" name="content" type="text/plain"> <c:out value="${information.contentStr}" escapeXml="false"/></script>
